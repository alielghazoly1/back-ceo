...
const getSaleInvoiceById = async (req, res) => {
  try {
    ...
    if (!invoice)
      invoice = await SaleInvoice.findOne({ docNumber: id }).sort({
        createdAt: -1,
      });
    if (!invoice)
...
const checkDocNumber = async (req, res) => {
  try {
    ...
    const exists = await SaleInvoice.findOne(query).select(
      'invoiceNumber docNumber season',
    );
    ...
    if (exists) {
      ...
      if (
        exists.season &&
        exists.season.toString() !== targetSeason?._id?.toString()
      ) {
  ...
  try {
    ...
    for (const saleItem of items) {
      ...
      if (!dbItem)
        return res
          .status(404)
          .json({ message: `الصنف ${saleItem.itemCode} مش موجود` });
      ...
      for (const saleItem of invoice.items) {
        await Item.findByIdAndUpdate(saleItem.item, {
          $inc: {
            ...
            [`stock.${invoice.warehouse}.weight`]: calcItemTotalWeight(
              saleItem.quantity,
              saleItem.weight,
            ),
          },
...
const cancelSaleInvoice = async (req, res) => {
  try {
    ...
    if (invoice.status === 'approved')
      return res
        .status(400)
        .json({ message: 'مينفعش تلغي فاتورة موافق عليها — عدّلها الأول' });
