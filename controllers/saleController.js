...
const { recordSaleInvoice } = require('../utils/treasuryHelper');
...
const createSaleInvoice = async (req, res) => {
  try {
    ...
    await audit(req.user, 'invoice_created', 'SaleInvoice', invoice._id, invoice.invoiceNumber, { customerName, totalAmount, paymentMethod: paymentMethod || 'credit' });
...
const updateSaleInvoice = async (req, res) => {
  try {
    ...
    if (wasApproved) {
      ...
      await Treasury.deleteMany({ referenceId: invoice._id, referenceModel: 'SaleInvoice' });
    }
...
const forceEditSaleInvoice = async (req, res) => {
  try {
    ...
    if (invoice.status === 'approved') {
      ...
      await StockMovement.deleteMany({ referenceId: invoice._id });
      // ── حذف سجلات الخزنة القديمة عشان متتسجلش مرتين عند الموافقة التانية ──
      await Treasury.deleteMany({ referenceId: invoice._id, referenceModel: 'SaleInvoice' });
    }
    ...
    await audit(req.user, 'invoice_edited', 'SaleInvoice', invoice._id, invoice.invoiceNumber, { editNotes: editNotes || '', customerName: invoice.customerName, totalAmount: invoice.totalAmount });
...
const approveSaleInvoice = async (req, res) => {
  try {
    ...
    await recordSaleInvoice(invoice, req.user);

    await audit(req.user, 'invoice_approved', 'SaleInvoice', invoice._id, invoice.invoiceNumber, { customerName: invoice.customerName, totalAmount: invoice.totalAmount });
...
const suspendSaleInvoice = async (req, res) => {
  try {
    ...
    await invoice.save();
    await audit(req.user, 'invoice_suspended', 'SaleInvoice', invoice._id, invoice.invoiceNumber);
...
const cancelSaleInvoice = async (req, res) => {
  try {
    ...
    await audit(req.user, 'invoice_cancelled', 'SaleInvoice', invoice._id, invoice.invoiceNumber, { customerName: invoice.customerName });
